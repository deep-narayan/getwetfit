...
$stmt->execute($params);
$specialResult = $stmt->fetchAll();


function extractYouTubeID($url) {
    // Handles formats: watch?v=, youtu.be/, embed/, etc.
...
<section class="gallery-events">
    <div class="container">
        ...
        <!-- Events Calendar -->
        <?php
            include 'includes/upcomingEvents/upcomingEvents.php';
        ?>
            ...
            <div class="portfolio">
                <div class="container">
                    ...
                    <div class="row portfolio-container">
                        <?php
                           $query = $conn->prepare("SELECT * FROM galleryurl");
                           $query->execute();
                           $result = $query->fetchAll();

                           foreach($result as $res){
                                echo "
                                <div class='col-lg-4 col-md-6 col-sm-12 portfolio-item ".$res['sessionType']." wow fadeInUp' >
                                    <div class='portfolio-wrap'>
                                        <a href=".$res[1]." data-lightbox='portfolio'>
                                            <img src=".$res[1]." alt='Portfolio Image'  loading='lazy'>
                                        </a>
                                    </div>
                                </div>";
                           }
            ...
            <div class="video-session-gallery">
                <div class="container">
                    ...
                    <?php 
                        ...
                        // } else {
                        //     $thumbnail_url = "./assets/images/default-thumbnail.jpg"; // fallback
                        // }

                    ?>
                    <div class="row video-scroll-container">
                        <?php foreach($result as $vres): ?>
                            <?php
                                $videoUrl = $vres['videoUrl'];
                            ...
                            ?>
                            <div class="col-lg-4 col-md-6 col-sm-12 video-session-item <?= $vres['sessionType'] ?>">
                                <div class="video-session-wrap position-relative">
                                    <!-- Background image (optional) -->
                                <img src="<?= $thumbnail_url ?>" alt="Video Thumbnail"  loading="lazy" class="thumbnail">

                                    <!-- Glightbox trigger -->
                                    <a href="<?= $videoUrl ?>" class="glightbox d-flex justify-content-center align-items-center w-100 h-100 position-absolute top-0 start-0" data-type="video">
        ...
        <?php
            include 'includes/specialEvents/upcomingSpecialEvents.php';
        ?>
